...
const qSVG = {
  create: function (id, shape, attrs) {
    ...
    if (id != "none") {
      document.getElementById(id)?.append(shapeElement);
    }
  ...
  polygonIntoWalls: function (vertex, surface) {
    ...
    for (let i = 0; i < wall.length; i++) {
      ...
      const edgeThicknessX = (window.editorVars.WALLS[edge.segment].thick / 2) * Math.sin(angleEdge);
      const edgeThicknessY = (window.editorVars.WALLS[edge.segment].thick / 2) * Math.cos(angleEdge);
      const nextEdgeThicknessX = (window.editorVars.WALLS[nextEdge.segment].thick / 2) * Math.sin(angleNextEdge);
      const nextEdgeThicknessY = (window.editorVars.WALLS[nextEdge.segment].thick / 2) * Math.cos(angleNextEdge);
  ...
  junctionList: function () {
    ...
    for (let i = 0; i < window.editorVars.WALLS.length; i++) {
      const equation1 = qSVG.createEquation(
        window.editorVars.WALLS[i].start.x,
        window.editorVars.WALLS[i].start.y,
        window.editorVars.WALLS[i].end.x,
        window.editorVars.WALLS[i].end.y
      );
      for (let v = 0; v < window.editorVars.WALLS.length; v++) {
        if (v != i) {
          const equation2 = qSVG.createEquation(
            window.editorVars.WALLS[v].start.x,
            window.editorVars.WALLS[v].start.y,
            window.editorVars.WALLS[v].end.x,
            window.editorVars.WALLS[v].end.y
          );
          let intersec;
          if ((intersec = qSVG.intersectionOfEquations(equation1, equation2))) {
            if (
              (window.editorVars.WALLS[i].end.x == window.editorVars.WALLS[v].start.x &&
                window.editorVars.WALLS[i].end.y == window.editorVars.WALLS[v].start.y) ||
              (window.editorVars.WALLS[i].start.x == window.editorVars.WALLS[v].end.x &&
                window.editorVars.WALLS[i].start.y == window.editorVars.WALLS[v].end.y)
            ) {
              if (
                window.editorVars.WALLS[i].end.x == window.editorVars.WALLS[v].start.x &&
                window.editorVars.WALLS[i].end.y == window.editorVars.WALLS[v].start.y
                ...
                junction.push({
                  segment: i,
                  child: v,
                  values: [window.editorVars.WALLS[v].start.x, window.editorVars.WALLS[v].start.y],
                  type: "natural",
                });
              }
              if (
                ...
                junction.push({
                  segment: i,
                  child: v,
                  values: [window.editorVars.WALLS[i].start.x, window.editorVars.WALLS[i].start.y],
                  type: "natural",
                });
              }
            } else {
        ...
        for (let ft = fr + 1; ft < vertex[ss].child.length; ft++) {
          if (fr != ft && typeof vertex[ss].child[fr] != "undefined") {
            ...
            if (
              ...
              const dOne = qSVG.gap(vertex[ss], vertex[vertex[ss].child[ft].id]);
              const dTwo = qSVG.gap(vertex[ss], vertex[vertex[ss].child[fr].id]);
              if (dOne > dTwo) {
                toClean.push(ft);
              } else {
                toClean.push(fr);
    ...
    for (let jc = 0; jc < edgesChild.length; jc++) {
      ...
      if (WAYS.length > 0) {
        ...
        if (vertex[bestVertex].bypass == 0) {
          ...
          const realCoords = qSVG.polygonIntoWalls(vertex, tempSurface, segment);
        ...
        if (qSVG.rayCasting({ x: posX, y: posY }, polygon)) {
          ...
          for (let ii = 0; ii < insideArray.length; ii++) {
            if (
              qSVG.rayCasting(
                { x: posX, y: posY },
                window.editorVars.ROOM[insideArray[ii]].coordsOutside
              )
            ) {
              found = false;
              break;
